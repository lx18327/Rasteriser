...
glm::vec3 GetPixelSDL(SDL_Surface *textureSurface, int x, int y);
...
glm::vec3 GetPixelSDL(SDL_Surface *textureSurface, int x, int y){
  int bytes_per_pixel = textureSurface->format->BytesPerPixel;

  Uint8 *p = (Uint8 *)textureSurface->pixels + y * textureSurface->pitch + x * bytes_per_pixel;
  ...
  uint8_t r, g, b;
  SDL_GetRGB(sdl_pixel_value, textureSurface->format, &r, &g, &b);
  std::cout << "apa" << std::endl;
 ...
 // pixel_value.g = g;
  //pixel_value.b = b;
 std::cout << pixel_value.r << std::endl;
 std::cout << pixel_value.g << std::endl;
 std::cout << pixel_value.b << std::endl;
  return pixel_value;
}
